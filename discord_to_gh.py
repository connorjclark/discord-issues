...
async def process_thread(
    guild: discord.Guild, channel: discord.ForumChannel, thread: discord.Thread
):
    ...
    for message in messages:
        if (
            previous_message == None
            or previous_message.author.id != message.author.id
            or message.created_at - previous_message.created_at > timedelta(minutes=1)
        ):
...
async def on_ready():
    ...
    for issue in issues:
        ...
        try:
            chat_log = re.sub(
                r'<(@|#|@!|@&)(\d+)>', replace_id_tags, issue.content
            ).strip()
    ...
    for thread in converted_threads:
        ...
        with open(f'issues/{thread.gh_id}.md', 'w') as f:
            ...
            print(
                f'Source: #{thread.issue.channel_name} https://discord.com/channels/{ZC_GUILD_ID}/{thread.discord_thread_id}',
                file=f,
            )
            print('\n', file=f)
